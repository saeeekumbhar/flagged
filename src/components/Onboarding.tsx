...
type Step = 'welcome' | 'name' | 'type' | 'commute' | 'food' | 'delivery' | 'device' | 'seed' | 'reveal';

const STEPS: Step[] = ['welcome', 'name', 'type', 'commute', 'food', 'delivery', 'device', 'seed', 'reveal'];

const SEEDS = [
  { id: 'sunflower', emoji: '🌻', label: 'Sunflower', desc: 'Bold and energetic' },
  { id: 'fern', emoji: '🌿', label: 'Fern', desc: 'Calm and steady' },
  { id: 'blossom', emoji: '🌸', label: 'Cherry Blossom', desc: 'Gentle and flourishing' },
];
...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const [selectedSeed, setSelectedSeed] = useState<string>('fern');
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="flex flex-col items-center text-center gap-8">
            ...
            <div>
              <h1 className="text-display text-4xl font-bold text-[#1F3D20] mb-3 leading-tight">
                Every great garden<br />starts with a choice.
              </h1>
              <p className="text-[#5A8070] text-base leading-relaxed">
                FLAGGED helps you build better habits — one green flag at a time.
              </p>
            </div>
        ...
        return (
          <div className="flex flex-col gap-6">
            <div className="text-center">
              ...
              <h2 className="text-display text-3xl font-bold text-[#1F3D20] mb-2">
                What should we call you?
              </h2>
              <p className="text-[#8A8070] text-sm">Your garden will be uniquely yours.</p>
            </div>

            <input
              type="text"
              className="soft-input text-center text-lg"
            ...
            {profile.name && (
              ...
              >
                Hi {profile.name?.split(' ')[0]}! Let's grow something beautiful 🌿
              </motion.p>
            )}
            ...
            <motion.button
              ...
              style={{ opacity: profile.name ? 1 : 0.5 }}
              whileTap={{ scale: 0.97 }}
            >
              Continue
            </motion.button>
        ...
        return (
          <div className="flex flex-col gap-6">
            <div className="text-center">
              ...
              <p className="text-[#8A8070] text-sm">Your commute shapes your garden.</p>
            </div>
            <div className="flex flex-col gap-3">
              <ChoiceBtn emoji="🚶" label="Walk or cycle" desc="Your garden loves this 🌿"
                onClick={() => { setProfile({ ...profile, commuteMethod: 'walk' }); nextStep(); }} />
              <ChoiceBtn emoji="🚌" label="College bus" desc="Smart and steady"
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              {profile.userType === 'day_scholar' ? (
                <>
                  <ChoiceBtn emoji="🏠" label="Home cooked" desc="Rooted and nourishing"
                    onClick={() => { setProfile({ ...profile, foodPreferences: 'home' }); nextStep(); }} />
                  <ChoiceBtn emoji="🍽️" label="College canteen" desc="Campus community vibes"
        ...
        return (
          <div className="flex flex-col gap-6">
            <div className="text-center">
              <div className="text-4xl mb-4">📦</div>
              <h2 className="text-display text-3xl font-bold text-[#1F3D20] mb-2">
                How often does delivery call?
              </h2>
              <p className="text-[#8A8070] text-sm">Be honest — your garden can handle it.</p>
            </div>

            <div className="soft-card p-6">
              <div className="flex justify-between items-center mb-4">
                <span className="text-3xl font-mono font-bold text-[#5A8F5A]">
                  {profile.deliveryFrequency}×
                </span>
      ...
      case 'device':
        return (
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              <ChoiceBtn emoji="✅" label="No, I unplug them" desc="Mindful and efficient 🌿"
        ...
        return (
          <div className="flex flex-col gap-6">
            <div className="text-center">
              <div className="text-4xl mb-4">🪴</div>
              <h2 className="text-display text-3xl font-bold text-[#1F3D20] mb-2">
                Choose your garden seed
              </h2>
              <p className="text-[#8A8070] text-sm">Each grows differently. All are beautiful.</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {SEEDS.map(seed => (
                <motion.button
                  key={seed.id}
                  className={`btn-choice flex items-center gap-4 text-left ${selectedSeed === seed.id ? 'selected' : ''}`}
                  onClick={() => setSelectedSeed(seed.id)}
                  whileTap={{ scale: 0.97 }}
                >
                  <span className={`text-4xl ${selectedSeed === seed.id ? 'plant-float' : ''}`}>{seed.emoji}</span>
                  <div>
                    <div className="font-semibold text-[#1E1A16]">{seed.label}</div>
                    <div className="text-sm text-[#8A8070]">{seed.desc}</div>
                  </div>
                  {selectedSeed === seed.id && (
                    <motion.span
                      initial={{ scale: 0 }} animate={{ scale: 1 }}
                      className="ml-auto text-[#5A8F5A] text-xl"
                    >✓</motion.span>
                  )}
                </motion.button>
              ))}
      ...
      case 'reveal':
        const seedEmoji = SEEDS.find(s => s.id === selectedSeed)?.emoji || '🌱';
        return (
          <div className="flex flex-col items-center text-center gap-8">
            <motion.div
            ...
            >
              ...
              <div className="w-40 h-40 rounded-full flex items-center justify-center relative"
                style={{ background: 'linear-gradient(135deg, #E4EDE0 0%, #FDF6EC 100%)', boxShadow: '0 12px 40px rgba(90,143,90,0.25)' }}>
                <span className="text-7xl plant-float">{seedEmoji}</span>
              </div>
            </motion.div>

            <div>
              <motion.p
                className="text-sm font-semibold text-[#7BA87A] uppercase tracking-widest mb-2"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
              >
                Welcome to your garden,
              </motion.p>
              <motion.h1
                className="text-display text-4xl font-bold text-[#1F3D20] mb-4"
              ...
              >
                <span>🌿</span> Mixed Flags Era
              </motion.div>
              <motion.p
                className="text-[#8A8070] text-sm mt-4 leading-relaxed"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3 }}
              >
                Your garden is just beginning. Every good habit<br />you build will make it bloom.
              </motion.p>
            </div>
            ...
            >
              <span>{seedEmoji}</span> Enter My Garden
            </motion.button>
          </div>
        );
  ...
  return (
    <div
      ...
      style={{
        background: 'radial-gradient(ellipse 100% 60% at 50% 0%, rgba(196,217,188,0.35) 0%, transparent 65%), #FDFAF5'
      }}
    >
      {/* Progress dots */}
      {showDots && (
        <div className="mb-6">
          <ProgressDots current={dotIndex} total={STEPS.length - 2} />
        </div>
      )}
